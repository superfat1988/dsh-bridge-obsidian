/**
 * obsidian bridge host Host adapter.
 *
...
/** Structural subset of obsidian bridge host's Host TypertGateway service. */
...
/** Structural subset of obsidian bridge host's Host Connection service. */
...
/** Build the obsidian bridge host Host implementation. */
...
class RemoteHostApi implements BridgeHostApi {
...
class EventGeneration {
  ...
  async dispose(): Promise<void> {
    ...
    this.disposed = true
    this.followAbort?.abort(new Error('obsidian bridge event generation closed'))
    this.lifetime.abort(new Error('obsidian bridge event generation closed'))
    this.queue.end()
  ...
  private async openSessionFollow(
    ...
    this.followAbort?.abort(new Error('obsidian bridge Session follower replaced'))
    const controller = new AbortController()
  ...
  switch (call.method) {
    ...
    default:
      return {
        error: {
          code: 'not-found',
          message: `obsidian bridge Host method ${JSON.stringify(call.method)} is unavailable`,
          details: {},
        },
      }
  }
