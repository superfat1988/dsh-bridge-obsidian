 ...
 *
 * The token authenticates the Obsidian client against the bridge WebSocket.
 ...
 * Unlike the browser bridge, Obsidian clients never get a loopback shortcut:
 * the Obsidian plugin has a settings field, so the token is always required.
 *
 * @module
 */
...
export const TOKEN_FILE_NAME = 'obsidian-bridge-token'
 ...
 * @returns absolute path like `~/.dsh/obsidian-bridge-token`.
 */
